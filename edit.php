...
try {
    ...
    $stmt = $pdo->prepare("SELECT id, name, category_id, supplier_id, price, stock FROM products WHERE id = :id");
    $stmt->execute(['id' => $id]);
    $product = $stmt->fetch();
...
<div class="container">
    ...
    <form class="data-form" method="POST" action="edit.php?id=<?php echo urlencode((string) $id); ?>">
        <label for="name">Nama Produk</label>
        <input type="text" id="name" name="name" value="<?php echo htmlspecialchars((string) $name); ?>" maxlength="150" required>
